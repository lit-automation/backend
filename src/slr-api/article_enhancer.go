...
func gatherAdditionalInfo(client crossref.Client, bibClient bibtex.Client, article ArticleEnhancement) {
	err := database.Transact(DB.DB, func(tx *gorm.DB) error {
		...
		articleFromDB, err := articleDB.Get(context.Background(), article.ID)
		if err != nil {
		...
		if err != nil {
			...
			err = articleDB.Update(context.Background(), articleFromDB)
			if err != nil {
				log.WithError(err).WithField(logfields.ArticleID, article.ID).Error("unable to update article")
			}
		}
		...
		updateArticleInfo(articleDB, articleFromDB, artInfo)
		if articleFromDB.Doi != "" {
			updateBibTex(articleDB, bibClient, articleFromDB)
		}
		return nil
	})
	if err != nil {
...
func updateBibTex(articleDB *models.ArticleDB, bibClient bibtex.Client, articleFromDB *models.Article) {
...
func updateArticleInfo(articleDB *models.ArticleDB, articleFromDB *models.Article, artInfo *crossref.ArticleInfo) {
